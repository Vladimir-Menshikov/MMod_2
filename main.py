...
class Model:
    def __init__(self, n, m, lambd, mu, nu, queries, eps):
        self.n = n
        self.m = m
        self.lambd = lambd
        self.mu = mu
    ...
    def show_empirical_stats(self):
        ...
        avg_queue_length = 0
        for s in range(self.n + 1, self.n + self.m + 1):
    ...
    def show_theoretical_stats(self):
        ...
        p0 = 1 / (nsum(lambda k: ro ** k / fac(k), [0, self.n]) + (ro ** self.n / fac(self.n)) * nsum(
            lambda i: ro ** i / nprod(lambda l: (self.n + l * self.nu / self.mu), [1, i]), [1, self.m]))
        ...
        for s in range(1, self.m + 1):
            ps = ro ** (self.n + s) * p0 / (fac(self.n) * nprod(lambda l: self.n + l * self.nu / self.mu, [1, s]))
            probs.append(ps)
    ...
    def show_stats(self):
        ...
        print(f'PARAMS: n={self.n}, m={self.m}, lambda={self.lambd}, mu={self.mu}, nu={self.nu}')
        ...
        ax[0].title.set_text(
            f'Empirical probabilities (n={self.n}, m={self.m}, lambda={self.lambd}, mu={self.mu}, nu={self.nu})')
        ax[0].hist(list(np.arange(0, len(e_probs), 1)), weights=e_probs)
        ax[1].title.set_text(
            f'Theoretical probabilities (n={self.n}, m={self.m}, lambda={self.lambd}, mu={self.mu}, nu={self.nu})')
